...
installation pinned beside this script and serves an existing site
...
class _ResourceParser(html.parser.HTMLParser):
    def __init__(self) -> None:
        ...
        self.has_embedded_script = False
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        for attribute in FETCHABLE_ATTRIBUTES.get(tag, ()):
        ...
        for attribute in ("fill", "stroke", "filter", "mask", "clip-path", "marker"):
            if value := attributes.get(attribute):
                self.inline_css.append(value)
        if srcdoc := attributes.get("srcdoc"):
            embedded = _ResourceParser()
            embedded.feed(srcdoc)
            self.references.extend(embedded.references)
            self.inline_css.extend(embedded.inline_css)
            self.has_script = self.has_script or embedded.has_script
            self.has_embedded_script = self.has_embedded_script or embedded.has_script
        if tag.lower() == "style":
            self._in_style = True
...
def collect_asset_paths(html_path: Path, site_dir: Path) -> tuple[Path, ...]:
    ...
    for quarto_resource_dir in (
        html_path.with_name(f"{html_path.stem}_files"),
        site_dir / "site_libs",
    ):
        if not quarto_resource_dir.is_dir():
            continue
    ...
    while pending:
        ...
        if suffix == ".css":
            references = css_references(asset.read_text(encoding="utf-8"))
        elif suffix in {".html", ".htm", ".svg"}:
            embedded = _ResourceParser()
            embedded.feed(asset.read_text(encoding="utf-8"))
            references = embedded.references
            for inline_css in embedded.inline_css:
                references.extend(css_references(inline_css))
        else:
            continue
...
def cache_bypass_reason(html_path: Path, site_dir: Path) -> str | None:
    """Do not guess the dependencies loaded by executable embedded content."""
    resource_dirs = (html_path.with_name(f"{html_path.stem}_files"), site_dir / "site_libs")
    pending = [html_path]
    examined: set[Path] = set()
    while pending:
        asset = pending.pop()
        if asset in examined:
            continue
        examined.add(asset)
        suffix = asset.suffix.lower()
        references: list[str] = []
        if suffix in {".html", ".htm", ".svg"}:
            ...
            if (asset != html_path and parser.has_script) or parser.has_embedded_script:
                return f"scripted embedded document {asset.name}"
            references.extend(parser.references)
            for inline_css in parser.inline_css:
                references.extend(css_references(inline_css))
        elif suffix == ".css":
            references.extend(css_references(asset.read_text(encoding="utf-8")))
        if suffix in {".js", ".mjs", ".cjs"} and not any(
            asset.is_relative_to(directory) for directory in resource_dirs
        ):
            return f"script outside the deck dependency tree: {asset.name}"
        for reference in references:
            path = _local_reference_path(reference, asset, site_dir)
            if path is not None and path not in examined:
                pending.append(path)
    return None
...
def resolve_renderer(renderer_dir: Path, override: Path | None) -> RendererInfo:
    package_json_path = renderer_dir / "package.json"
    lock_candidates = (renderer_dir / "bun.lock", renderer_dir / "package-lock.json")
    ...
    if not package_json_path.is_file() or lock_path is None:
        raise RenderError(
            "renderer metadata is incomplete; expected package.json and "
    ...
    if not isinstance(expected_version, str) or not re.fullmatch(
        ...
        raise RenderError("renderer package.json must pin an exact DeckTape version")
    ...
    if override is not None:
        ...
        executable = renderer_dir / "node_modules" / ".bin" / "decktape"
        installed_package = renderer_dir / "node_modules" / "decktape" / "package.json"
        if not executable.is_file() or not os.access(executable, os.X_OK):
            raise RenderError(
                "pinned DeckTape is not installed; run `make pdf-setup` or "
                f"`bun install --frozen-lockfile --cwd {renderer_dir}` before rendering"
            )
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--no-cache", action="store_true", help="always capture dynamic or time-dependent decks"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        for html_path in decks:
            ...
            bypass = cache_bypass_reason(html_path, site_dir)
            if bypass:
                print(
                    f"cache disabled for {html_path.relative_to(site_dir)}: {bypass}",
                    flush=True,
                )
            for mode in modes:
